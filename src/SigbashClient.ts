...
export class SigbashClient {
  ...
  async signPSBT(options: SignPSBTOptions): Promise<SignPSBTResult> {
    ...
    if (typeof options.mockedTime === 'number' && Number.isFinite(options.mockedTime) && options.mockedTime > 0) {
      const setMockedTimeFn = (globalThis as Record<string, unknown>)['SetMockedTimeForTesting'] as
    ...
    try {
      ...
      result = await wasmFn(psbtBase64, options.kmcJSON, 0, network, progressCallback, seedHex, arkadeCtxJSON, options.finalizePsbt ? 1 : 0);
    } catch (err) {
      ...
      if (mockedTimeApplied) {
        const resetMockedTimeFn = (globalThis as Record<string, unknown>)['ResetMockedTimeForTesting'] as
          | (() => unknown)
          | undefined;
